...
function genFragment(
  ...
  lastList: string,
  inEntity: number,
  customBlockChunkGenerator: ?CustomChunkGenerator,
  customInlineChunkGenerator: ?CustomChunkGenerator,
): Object {
  const nodeName = node.nodeName.toLowerCase();

  if (customBlockChunkGenerator) {
    const value = customBlockChunkGenerator(nodeName, node);
    if (value) {
  ...
  while (child) {
    const entityId = getEntityId(child, customInlineChunkGenerator);
    const { chunk: generatedChunk } = genFragment(
      ...
      lastList,
      entityId || inEntity,
      customBlockChunkGenerator,
      customInlineChunkGenerator,
    );
...
export default function htmlToDraft(
  ...
  const chunkData = getChunkForHTML(html, customBlockChunkGenerator, customInlineChunkGenerator);
  if (chunkData) {
